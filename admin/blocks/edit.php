...
if ('POST' === $_SERVER['REQUEST_METHOD'] && isset($_GET['id'])) {
	...
	$stmt = $conn->prepare('
	UPDATE blocks 
	SET block_name = ?, title = ?, block_content = ?, show_on_pages = ?, show_in_region = ?, sort = ?, css = ?, module_file = ?, visible_on = ?, number_of_records = ?, is_dynamic = ?, is_active = ?, updated_by = ?, 
	key_media_banner = ?, key_photo_gallery = ?, key_content_types = ?, key_categories = ?, key_tags = ? 
	WHERE key_blocks = ?
	');
	$stmt->bind_param('sssssisssiiiiiiiiii',
	$_POST['block_name'],
	$_POST['title'],
	$_POST['block_content'],
	$_POST['show_on_pages'],

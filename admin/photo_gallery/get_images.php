...
$res = $conn->query('
		SELECT photogal.key_image, media_library.file_url AS banner_url 
		FROM photo_gallery_images photogal 
		LEFT JOIN media_library ON photogal.key_media_banner = media_library.key_media 
		WHERE key_photo_gallery = $keyPhotoGallery 
		ORDER BY sort_order
		');

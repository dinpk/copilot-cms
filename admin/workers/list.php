...
<form method="get">
	<input type="text" name="q" placeholder="Search workers..." value="<?= htmlspecialchars($_GET['q'] ?? '') ?>">
...
<table>
	...
	$keyWorkers = $row["key_workers"];
	$createdUpdated = $conn->query("SELECT a.key_workers, u1.username AS creator, u2.username AS updater 
		FROM workers a 
		LEFT JOIN users u1 ON a.created_by = u1.key_user
		LEFT JOIN users u2 ON a.updated_by = u2.key_user 
		WHERE key_workers = $keyWorkers")->fetch_assoc();
	  echo "<tr>
		<td>{$row['name']}</td>
		<td>{$row['email']}</td>
		<td>{$row['city']}</td>
		...
		<td class='record-action-links'>
		  <a href='#' onclick='editItem({$row['key_workers']}, \"get_worker.php\", [\"name\",\"email\",\"phone\",\"website\",\"url\",\"banner_image_url\",\"social_url_media1\",\"social_url_media2\",\"social_url_media3\",\"city\",\"state\",\"key_media_banner\",\"country\",\"description\",\"is_active\"])'>Edit</a> 
		  <a href='delete.php?id={$row['key_workers']}' onclick='return confirm(\"Delete this worker?\")' style='display:none'>Delete</a>
		</td>
	  </tr>";
	}
	$countSql = "SELECT COUNT(*) AS total FROM workers";
	if ($q !== '') {
...
<div id="modal" class="modal">
	...
	<h3 id="modal-title">Add Worker</h3>
	<form id="modal-form" method="post">
		<input type="hidden" name="key_workers" id="key_workers">
		...
		<button type="button" onclick="galleryImage_openMediaModal(document.querySelector('#key_workers').value)">Select Banner Image from Media Library</button><br>
